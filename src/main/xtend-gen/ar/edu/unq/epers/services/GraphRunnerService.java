...
public class GraphRunnerService {
  private static GraphDatabaseService graphDatabaseService;
  
  public static synchronized GraphDatabaseService getGraphDb() {
    GraphDatabaseService _xblockexpression = null;
    {
      boolean _equals = Objects.equal(GraphRunnerService.graphDatabaseService, null);
      if (_equals) {
        GraphDatabaseFactory _graphDatabaseFactory = new GraphDatabaseFactory();
        GraphDatabaseBuilder _newEmbeddedDatabaseBuilder = _graphDatabaseFactory.newEmbeddedDatabaseBuilder("/home/guidop/Desktop/baseNeo4j");
        GraphDatabaseService _newGraphDatabase = _newEmbeddedDatabaseBuilder.newGraphDatabase();
        GraphRunnerService.graphDatabaseService = _newGraphDatabase;
        Runtime _runtime = Runtime.getRuntime();
        final Runnable _function = new Runnable() {
          @Override
          public void run() {
            GraphDatabaseService _graphDb = GraphRunnerService.getGraphDb();
            _graphDb.shutdown();
          }
        };
      ...
      _xblockexpression = GraphRunnerService.graphDatabaseService;
    }
    ...
    try {
      T _xblockexpression = null;
      {
        ...
        try {
          ...
          {
            GraphDatabaseService _graphDb_1 = GraphRunnerService.getGraphDb();
